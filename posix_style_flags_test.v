...
fn test_pure_posix_short() {
	config, _ := flags.to_struct[Config](exe_and_posix_args, skip: 1, style: .short)!
...
fn test_pure_posix_multi_short_1() {
	config, _ := flags.to_struct[Config](posix_multi_short_args_1, style: .short)!
	assert config.verbosity == 5
...
fn test_pure_posix_multi_short_2() {
	config, _ := flags.to_struct[Config](posix_multi_short_args_2, style: .short)!
	assert config.verbosity == 5
	assert config.test == 'def'
	assert 'one' in config.device
	assert 'two' in config.device
...
fn test_pure_posix_multi_short_3() {
	config, _ := flags.to_struct[Config](posix_multi_short_args_3, style: .short)!
	assert config.verbosity == 5
	assert config.test == 'def'
	assert 'one' in config.device
	assert 'two' in config.device
...
fn test_pure_posix_multi_short_4() {
	config, _ := flags.to_struct[Config](posix_multi_short_args_4, style: .short)!
	assert config.verbosity == 5
	assert config.test == 'def'
	assert 'one' in config.device
	assert 'two' in config.device
...
fn test_pure_posix_multi_short_err_1() {
	if _, _ := flags.to_struct[Config](posix_multi_short_args_1_err, style: .short) {
...
fn test_pure_posix_short_no_exe() {
	config, _ := flags.to_struct[Config](exe_and_posix_args[1..], style: .short)!
	assert config.verbosity == 5
	assert config.test == 'abc'
	assert 'one' in config.device
	assert 'two' in config.device
...
fn test_pure_posix_short_with_tail() {
	config, _ := flags.to_struct[Config](exe_and_posix_args_with_tail, skip: 1, style: .short)!
...
fn test_pure_posix_short_with_tail_no_exe() {
	config, _ := flags.to_struct[Config](exe_and_posix_args_with_tail[1..], style: .short)!
	assert config.verbosity == 6
	assert config.test == 'abc'
	assert 'one' in config.device
	assert 'two' in config.device
